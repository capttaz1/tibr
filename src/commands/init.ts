...
export async function initHandler(name: string, preset: 'react-monorepo' | 'ts', pm: 'npm' | 'yarn' | 'pnpm') {
	...
	// cd into new workspace
	process.chdir(workspaceDir);

	// // 3) install & init all of our plugins
	...
	// // 		'ui', // ← now matches the actual project name
	...
	// await execa('nx', ['g', '@nx/storybook:configuration', 'ui'], { stdio: 'inherit' });
